...
export async function POST(req) {
  ...
  try {
    const { title, prompt, description, modelName, recaptchaToken } = await req.json();
    ...
    try {
      ...
      console.error('Error verifying token or creating prompt:', error);
      return NextResponse.json({ message: 'Unauthorized or error creating prompt' }, { status: 401 });
    }
  } catch (error) {
